...
if __name__ == '__main__':
    ...
    for i in range(3):
        rgb_bilateral_dst.append(my_normalize(my_bilateral(src_noise[:, :, i], 5, 3, 3)))
        rgb_gaussian_dst.append(my_normalize(my_filtering(src_noise[:, :, i], my_get_Gaussian2D_mask(5))))
        rgb_median_dst.append(my_median_filtering(noise_image[:, :, i], 10))
    ...
    ######################################################
    # YUV
    noise_yuv_image = cv2.cvtColor(noise_image, cv2.COLOR_BGR2YUV)
    yuv_bilateral_dst = noise_yuv_image.copy()
    yuv_gaussian_dst = noise_yuv_image.copy()
    yuv_median_dst = noise_yuv_image.copy()

    yuv_bilateral_dst[:, :, 0] = my_normalize(my_bilateral(noise_yuv_image[:, :, 0]/255, 5, 3, 3))
    yuv_gaussian_dst[:, :, 0] = my_normalize(my_filtering(noise_yuv_image[:, :, 0]/255, my_get_Gaussian2D_mask(5)))
    yuv_median_dst[:, :, 0] = my_median_filtering(noise_yuv_image[:, :, 0], 10)
